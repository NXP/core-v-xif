...
An issue request transaction is defined as the combination of all ``issue_req`` signals during which ``issue_valid`` is 1 and the ``hartid`` and ``id`` remain unchanged.
...
* Set ``issue_valid`` = 0.
* Keep ``issue_valid`` = 1, but change the ``hartid`` and/or ``id`` signal (and if desired change the other signals in ``issue_req``).
